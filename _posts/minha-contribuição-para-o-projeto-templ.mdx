---
title: "Adicionando `--ignore-pattern` ao watch do templ"
thumbnail: https://templ.guide/img/logo.svg
description: "Um estudo de caso sobre a adição da flag --ignore-pattern ao modo watch do templ, abordando limitações do RE2, decisões de usabilidade e o processo completo de contribuição open source."
date: "2025-01-10"
tags: [templ, go, open-source, cli, github]
---

Durante o uso do `templ generate -watch`, tive um problema recorrente:
arquivos **gerados automaticamente** (como `mage_output_file.go`) despertavam o modo watch, causando **regenerações e reloads desnecessários**.
...
Mas isso **não é verdade nesse contexto.**
...
* **Issue:** [Can we have a `--ignore-pattern` flag for watch mode?](https://github.com/a-h/templ/issues/1279)
...
O autor e mantenedor do projeto [a-h](https://github.com/a-h) concordou rapidamente.
...
3. Esperar validação do mantenedor do projeto
...
O `--ignore-pattern` resolve um problema concreto do modo watch sem alterar o comportamento existente. A solução é explícita, previsível e fácil de entender, tanto para quem usa quanto para quem mantém o código.

Os testes adicionados garantem que a flag funcione corretamente e que tenha precedência sobre o `--watch-pattern`, evitando regressões. Isso deixa a mudança segura de evoluir e simples de manter.

Todo o fluxo — issue, validação, implementação e PR — foi feito com comunicação clara e foco em qualidade. É o tipo de contribuição pequena em superfície, mas sólida em impacto.
